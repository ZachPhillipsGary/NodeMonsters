...
module.exports = {
  map: function(size) {
   ...
   tile.prototype.getTile = function () {
    var objectKey = "__NONE__";
    for (var key in this.objects) {
      console.log(this.objects[key]);
        objectKey =  this.objects[key];
    }
...
module.exports.map.prototype.getAbove = function (x,y) {
  console.log(this.mapArray.length,x,y)
 /*if (y > this.mapArray.length) {
    return this.mapArray[x][0];
  } 
  if (y < 0) {
    return this.mapArray[x][this.mapArray.length];
  } else {
...
module.exports.map.prototype.clearObjects = function() {
  for (var i = 0; i < this.mapArray.length; i++) {
     for (var j = 0; i < this.mapArray[i].length; j++) {
        for (var obj in this.mapArray[i][j].objects)
            delete this.mapArray[i][j].objects[obj];
      }
  }
}
module.exports.map.prototype.updatePlayer = function(player) {
    
     this.mapArray[player.x][player.y].objects[String(player.username)] = player.color; 
