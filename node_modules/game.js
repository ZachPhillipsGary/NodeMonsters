...
module.exports.map.prototype.movePlayer = function (x,y,direction,player) {
  ...
  if (this.mapArray[x][y].objects.hasOwnProperty(String(player))) {
    switch(expression) {
    case "up":
    if( this.mapArray[x][y-1].kind != 1) {
     this.mapArray[x][y-1].objects[String(player)] = this.mapArray[x][y].objects[String(player)];
     delete this.mapArray[x][y].objects[String(player)]; // remove reference to player on previous tile
   }
        break;
    case "down":
    if( this.mapArray[x][y+1].kind != 1) {
  ...
  for (var i = 0; i < this.mapArray.length; i++) {
    mapVals[i] = [];
      for (var l = 0; l < this.mapArray[i].length; l++) {
        if ( Object.keys(this.mapArray[i][l].objects).length > 0 ) {
          for (var key in this.mapArray[i][l].objects) {
            mapVals[i][l] = this.mapArray[i][l].objects[key];
          }
        } else {
